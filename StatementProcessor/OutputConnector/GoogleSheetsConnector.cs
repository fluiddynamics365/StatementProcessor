...
public class GoogleSheetsConnector(IConfiguration configuration, ILogger<StatementProcessorService> logger) : IOutputConnector
{
    ...
    {
        using (var stream = new FileStream("googlesheetsapi.json", FileMode.Open, System.IO.FileAccess.Read))
        {
    ...
    public void AddTransactions(IEnumerable<Transaction> transactions)
    {
        ...
        var transactionsToAdd = transactions.Where(transaction => !existingTransactions.Any(t => t[Literals.Fields.TransactionId].ToString() == transaction.UniqueId)).ToList();
